...
def boxcount_dimension_3d(points, n_scales=8, min_boxes_per_axis=4, return_fit=False):
    """
    ...
    return_fit : bool
        If True, also return dict with scales, counts and fit arrays.
    ...
    info : dict (optional)
        'eps', 'N_boxes', 'x', 'y', 'coeffs' for diagnostics
    """
    ...
    # Normalize to [0,1]^3 to make box sizes clean powers of two
    mins = P.min(axis=0)
    maxs = P.max(axis=0)
    span = maxs - mins
    # Avoid zero-span issues by inflating tiny spans
    span[span == 0] = 1.0
    Q = (P - mins) / span

    # Use box sizes: eps_k = 1 / (b_k), with b_k powers of 2
    # Start coarse (few boxes per axis) and go finer
    b0 = max(min_boxes_per_axis, 2)
    bs = np.array([b0 * (2**k) for k in range(n_scales)], int)
    ...
    # Fit: log N(eps) vs log(1/eps) = log b
    x = np.log(bs.astype(float))
    y = np.log(N_boxes + 1e-12)  # guard against log(0) in pathological cases
    coeffs = np.polyfit(x, y, 1)
    Dhat = coeffs[0]

    if return_fit:
        return Dhat, {"eps": eps, "N_boxes": N_boxes, "x": x, "y": y, "coeffs": coeffs}
    return Dhat

def _bounds_with_padding(stars, pad_fraction=0.02):
...
# -----------------------------------------------------------------------------
# Public API: Method 1 — Single-level (alpha-weighted or uniform) on a fixed grid
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Public API: Method 2 — Hierarchical global cascade (alpha-free)
# -----------------------------------------------------------------------------
